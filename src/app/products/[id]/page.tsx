...
export default function ProductDetails() {
  ...
  return (
    <div className="flex flex-col min-h-screen animate-in fade-in duration-500 bg-white">
      {/* Top Bar */}
      <div className="fixed top-0 left-0 right-0 z-50 px-4 py-4 flex justify-between items-center bg-gradient-to-b from-black/20 to-transparent">
        <button onClick={() => router.back()} className="w-10 h-10 rounded-full bg-white/80 backdrop-blur-md flex items-center justify-center shadow-sm">
      ...
      <div className="relative aspect-square w-full bg-gray-50">
        ...
        <div className="absolute bottom-6 right-6">
          <Badge className="bg-luxury-black text-white border-none px-4 py-1 rounded-full text-xs font-bold shadow-xl">
            {product.size}
          </Badge>
        </div>
      </div>
      ...
      <div className="flex-1 bg-white -mt-8 rounded-t-[3rem] p-8 space-y-8 relative z-10 shadow-[0_-20px_40px_rgba(0,0,0,0.05)]">
        ...
        <div className="space-y-4">
          <h3 className="text-lg font-black text-luxury-black border-r-4 border-primary pr-4">وصف العطر</h3>
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {product.ingredients.split('،').map((ing, i) => (
              <span key={i} className="bg-primary/5 text-primary text-xs font-bold px-4 py-2 rounded-xl">
                {ing.trim()}
              </span>
            ))}
          </div>
        ...
        {/* Trust Badges */}
        <div className="flex items-center justify-around py-6 border-y border-gray-50">
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 bg-white/80 backdrop-blur-md z-40 border-t border-gray-100 md:max-w-md md:mx-auto">
        <Button 
          ...
          className="w-full h-16 bg-[#25D366] hover:bg-[#1ebd5d] text-white rounded-2xl text-lg font-black gap-3 shadow-xl shadow-green-500/20"
        >
